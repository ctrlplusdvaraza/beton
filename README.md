...
Теперь мы перебираем не все индексы ```pos```, а только те, для которых ```partner``` больше ```pos```. Таким образом мы избавились от ```if```, но приобрели новый ```for```.
